...
```
kerberos KerbAddBindingCacheEntry --domain-name {name} --dc-address {Inet or NetBIOS address}
```
...
```
kerberos KerbAddBindingCacheEntryEx --domain-name {name} --dc-address {Inet or NetBIOS address} [--dc-flags {value}]
```
...
```
kerberos KerbAddExtraCredentials --domain-name {name} --user-name {name} --password {password} [--replace-cred | --remove-cred]
```
...
```
kerberos KerbCleanupMachinePkinitCreds [--luid {session id}]
```
...
```
kerberos KerbPinKdc --domain-name {name} --dc-name {name} [--dc-flags {value}]
```
...
```
kerberos KerbPrintCloudKerberosDebug
```
...
```
kerberos KerbPurgeBindingCache
```
...
```
kerberos KerbPurgeKdcProxyCache [--luid {session id}]
```
...
```
kerberos KerbPurgeTicketCache [--luid {session id}] [--server-name {name} --server-realm {name}]
```
...
```
kerberos KerbPurgeTicketCacheEx [--luid {session id}] [--all] [--client-name {name} --client-realm {name}] [--server-name {name} --server-realm {name}]
```
...
```
kerberos KerbQueryBindingCache
```
...
```
kerberos KerbQueryDomainExtendedPolicies --domain-name {name}
```
...
```
kerberos KerbQueryKdcProxyCache [--luid {session id}]
```
...
```
kerberos KerbQueryTicketCache [--luid {session id}]
```
...
```
kerberos KerbQueryTicketCacheEx [--luid {session id}]
```
...
```
kerberos KerbQueryTicketCacheEx2 [--luid {session id}]
```
...
```
kerberos KerbQueryTicketCacheEx3 [--luid {session id}]
```
...
```
kerberos KerbQueryS4U2ProxyCache [--luid {session id}]
```
...
```
kerberos KerbRetrieveEncodedTicket --target-name {server name} [--luid {session id}] [--ticket-flags {value}] [--cache-option {value}] [--enc-type {type}]
```
...
```
kerberos KerbRetrieveKeyTab --domain-name {name} --user-name {name} --password {password}
```
...
```
kerberos KerbRetrieveTicket --target-name {server name} [--luid {session id}] [--ticket-flags {value}] [--cache-option {value}] [--enc-type {type}]
```
...
```
kerberos KerbTransferCredentials --sluid {session id} --dluid {session id} [--cleanup-credentials] [--optimistic-logon]
```
