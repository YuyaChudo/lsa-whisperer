...
//
#include "lwdk/pkcs12.h"

ASN1module_t PKCS12_Module = NULL;

static int ASN1CALL ASN1Enc_AttributeSetValue_Set(ASN1encoding_t enc, ASN1uint32_t tag, AttributeSetValue_Set* val);
...
static int ASN1CALL ASN1Enc_AttributeSetValue(ASN1encoding_t enc, ASN1uint32_t tag, PAttributeSetValue* val);
...
static int ASN1CALL ASN1Enc_Attributes(ASN1encoding_t enc, ASN1uint32_t tag, PAttributes* val);
...
static int ASN1CALL ASN1Enc_AuthenticatedSafes(ASN1encoding_t enc, ASN1uint32_t tag, PAuthenticatedSafes* val);
...
static int ASN1CALL ASN1Enc_SafeContents(ASN1encoding_t enc, ASN1uint32_t tag, PSafeContents* val);
...
static int ASN1CALL ASN1Dec_AttributeSetValue_Set(ASN1decoding_t dec, ASN1uint32_t tag, AttributeSetValue_Set* val);
...
static int ASN1CALL ASN1Dec_AttributeSetValue(ASN1decoding_t dec, ASN1uint32_t tag, PAttributeSetValue* val);
...
static int ASN1CALL ASN1Dec_Attributes(ASN1decoding_t dec, ASN1uint32_t tag, PAttributes* val);
...
static int ASN1CALL ASN1Dec_AuthenticatedSafes(ASN1decoding_t dec, ASN1uint32_t tag, PAuthenticatedSafes* val);
...
static int ASN1CALL ASN1Dec_SafeContents(ASN1decoding_t dec, ASN1uint32_t tag, PSafeContents* val);
...
static void ASN1CALL ASN1Free_AttributeSetValue_Set(AttributeSetValue_Set* val);
...
static void ASN1CALL ASN1Free_AttributeSetValue(PAttributeSetValue* val);
...
static void ASN1CALL ASN1Free_Attributes(PAttributes* val);
...
static void ASN1CALL ASN1Free_AuthenticatedSafes(PAuthenticatedSafes* val);
...
static void ASN1CALL ASN1Free_SafeContents(PSafeContents* val);
...
static const ASN1EncFun_t encfntab[22] = {
    (ASN1EncFun_t)ASN1Enc_AttributeSetValue_Set,
...
static const ASN1DecFun_t decfntab[22] = {
    (ASN1DecFun_t)ASN1Dec_AttributeSetValue_Set,
...
static const ASN1FreeFun_t freefntab[22] = {
    (ASN1FreeFun_t)ASN1Free_AttributeSetValue_Set,
...
static const ULONG sizetab[22] = {
    SIZE_PKCS12_Module_PDU_0,
    SIZE_PKCS12_Module_PDU_1,
    SIZE_PKCS12_Module_PDU_2,
    SIZE_PKCS12_Module_PDU_3,
    SIZE_PKCS12_Module_PDU_4,
    SIZE_PKCS12_Module_PDU_5,
    SIZE_PKCS12_Module_PDU_6,
    SIZE_PKCS12_Module_PDU_7,
    SIZE_PKCS12_Module_PDU_8,
    SIZE_PKCS12_Module_PDU_9,
    SIZE_PKCS12_Module_PDU_10,
    SIZE_PKCS12_Module_PDU_11,
    SIZE_PKCS12_Module_PDU_12,
    SIZE_PKCS12_Module_PDU_13,
    SIZE_PKCS12_Module_PDU_14,
    SIZE_PKCS12_Module_PDU_15,
    SIZE_PKCS12_Module_PDU_16,
    SIZE_PKCS12_Module_PDU_17,
    SIZE_PKCS12_Module_PDU_18,
    SIZE_PKCS12_Module_PDU_19,
    SIZE_PKCS12_Module_PDU_20,
    SIZE_PKCS12_Module_PDU_21,
};

ASN1int32_t MacData_macIterationCount_default = 1;

void ASN1CALL PKCS12_Module_Startup() {
    PKCS12_Module = ASN1_CreateModule(0x10000, ASN1_BER_RULE_DER, ASN1FLAGS_NOASSERT, 22, (const ASN1GenericFun_t*)encfntab, (const ASN1GenericFun_t*)decfntab, freefntab, sizetab, 0x73636b70);
}

void ASN1CALL PKCS12_Module_Cleanup() {
    ASN1_CloseModule(PKCS12_Module);
    PKCS12_Module = NULL;
}

static int ASN1CALL ASN1Enc_AttributeSetValue_Set(ASN1encoding_t enc, ASN1uint32_t tag, AttributeSetValue_Set* val) {
    if (!ASN1BEREncOpenType(enc, val))
        return 0;
    return 1;
}

static int ASN1CALL ASN1Dec_AttributeSetValue_Set(ASN1decoding_t dec, ASN1uint32_t tag, AttributeSetValue_Set* val) {
    if (!ASN1BERDecOpenType(dec, val))
        return 0;
    return 1;
}
...
static int ASN1CALL ASN1Enc_AttributeSetValue(ASN1encoding_t enc, ASN1uint32_t tag, PAttributeSetValue* val) {
    ...
    for (f = *val; f; f = f->next) {
        if (!ASN1DEREncNewBlkElement(pBlk, &enc2))
            return 0;
        if (!ASN1BEREncOpenType(enc2, &f->value))
            return 0;
...
static int ASN1CALL ASN1Dec_AttributeSetValue(ASN1decoding_t dec, ASN1uint32_t tag, PAttributeSetValue* val) {
    ...
    while (ASN1BERDecNotEndOfContents(dd, di)) {
        if (!ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PAttributeSetValue)ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
        if (!ASN1BERDecOpenType(dd, &(*f)->value))
            return 0;
        f = &(*f)->next;
    }
    *f = NULL;
...
static void ASN1CALL ASN1Free_AttributeSetValue(PAttributeSetValue* val) {
    PAttributeSetValue f = 0, ff = 0;
    if (val) {
        for (f = *val; f; f = ff) {
            ASN1open_free(&f->value);
            ff = f->next;
            ASN1Free(f);
        }
    }
...
static int ASN1CALL ASN1Enc_Attributes(ASN1encoding_t enc, ASN1uint32_t tag, PAttributes* val) {
    ...
    for (f = *val; f; f = f->next) {
        if (!ASN1DEREncNewBlkElement(pBlk, &enc2))
            return 0;
        if (!ASN1Enc_Attribute(enc2, 0, &f->value))
            return 0;
        if (!ASN1DEREncFlushBlkElement(pBlk))
            return 0;
    }
...
static int ASN1CALL ASN1Dec_Attributes(ASN1decoding_t dec, ASN1uint32_t tag, PAttributes* val) {
    ...
    while (ASN1BERDecNotEndOfContents(dd, di)) {
        if (!ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PAttributes)ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
        if (!ASN1Dec_Attribute(dd, 0, &(*f)->value))
            return 0;
        f = &(*f)->next;
    }
    *f = NULL;
...
static void ASN1CALL ASN1Free_Attributes(PAttributes* val) {
    PAttributes f = 0, ff = 0;
    if (val) {
        for (f = *val; f; f = ff) {
            ASN1Free_Attribute(&f->value);
            ff = f->next;
            ASN1Free(f);
        }
    }
...
static int ASN1CALL ASN1Enc_AuthenticatedSafes(ASN1encoding_t enc, ASN1uint32_t tag, PAuthenticatedSafes* val) {
    PAuthenticatedSafes f;
    ASN1uint32_t nLenOff;
    if (!ASN1BEREncExplicitTag(enc, tag ? tag : 0x10, &nLenOff))
        return 0;
    for (f = *val; f; f = f->next) {
        if (!ASN1Enc_ContentInfo(enc, 0, &f->value))
            return 0;
    }
    if (!ASN1BEREncEndOfContents(enc, nLenOff))
        return 0;
...
static int ASN1CALL ASN1Dec_AuthenticatedSafes(ASN1decoding_t dec, ASN1uint32_t tag, PAuthenticatedSafes* val) {
    ...
    while (ASN1BERDecNotEndOfContents(dd, di)) {
        if (!ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PAuthenticatedSafes)ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
        if (!ASN1Dec_ContentInfo(dd, 0, &(*f)->value))
            return 0;
        f = &(*f)->next;
    }
    *f = NULL;
...
static void ASN1CALL ASN1Free_AuthenticatedSafes(PAuthenticatedSafes* val) {
    PAuthenticatedSafes f = 0, ff = 0;
    if (val) {
        for (f = *val; f; f = ff) {
            ASN1Free_ContentInfo(&f->value);
            ff = f->next;
            ASN1Free(f);
        }
    }
...
static int ASN1CALL ASN1Enc_SafeContents(ASN1encoding_t enc, ASN1uint32_t tag, PSafeContents* val) {
    ...
    if (!ASN1BEREncExplicitTag(enc, tag ? tag : 0x10, &nLenOff))
        return 0;
    for (f = *val; f; f = f->next) {
        if (!ASN1Enc_SafeBag(enc, 0, &f->value))
            return 0;
    }
    if (!ASN1BEREncEndOfContents(enc, nLenOff))
        return 0;
...
static int ASN1CALL ASN1Dec_SafeContents(ASN1decoding_t dec, ASN1uint32_t tag, PSafeContents* val) {
    ...
    while (ASN1BERDecNotEndOfContents(dd, di)) {
        if (!ASN1BERDecPeekTag(dd, &t))
            return 0;
        if (!(*f = (PSafeContents)ASN1DecAlloc(dd, sizeof(**f))))
            return 0;
        if (!ASN1Dec_SafeBag(dd, 0, &(*f)->value))
            return 0;
        f = &(*f)->next;
    }
    *f = NULL;
...
static void ASN1CALL ASN1Free_SafeContents(PSafeContents* val) {
    PSafeContents f = 0, ff = 0;
    if (val) {
        for (f = *val; f; f = ff) {
            ASN1Free_SafeBag(&f->value);
            ff = f->next;
            ASN1Free(f);
        }
    }
